...
int main(void){
	...
	double L00 = 16000;
	L12=L00/RR/T0;             //無次元化
	// kapa_c2=1.0e-12/b1/b1/RR/T0; //無次元化
	kapa_c2=5.0e-12/b1/b1/RR/T0; //無次元化
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			...
			// Cp[i][j]=0.66e3*den[i][j]*b1*b1*b1/KB*c2h[i][j]+1227.0*den[i][j]*b1*b1*b1/KB*(1.0-c2h[i][j]);
			Cp[i][j] = (6.05e3*0.66e3*c2h[i][j]+0.232*1227.0*(1.0-c2h[i][j]))*b1*b1*b1/KB;        //[J/kgK]×[kg/m3]=[J/Km3]→無次元化
			//printf("%d %d %e\n",i,j,Cp[i][j]);
		}
	}
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			...
			// c2=c2h[i][j];  c1=1.-c2; if(c1<=0.){c1=1.0e-06;} //正則溶体
			c2=c2h[i][j];  c1=1.-c2; if(c1<0.){c1=0.0;}         //四次関数
			...
			//--- 化学拡散ポテンシャルの計算 --------------------------------------------
			// kai2_chem[i][j]=(log(c2)-log(c1))+L12*(c1-c2); //正則溶体              //無次元化
			//kai2_chem[i][j]=L12*(c1-c2);
			kai2_chem[i][j]=(2. * L12)*c1*c2*(c1-c2); //四次関数
