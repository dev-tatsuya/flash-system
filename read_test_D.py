...
fp = open("dc_dev/bin/test_D.bin", "rb")
...
path = 'dc_dev/output/test_D/%s' % now
os.makedirs(path, exist_ok=True)

while pg.run():
    ind = (pg.totaltick) % cc.shape[0]

    pg.transform_blit_cmap(cc[ind], 2.25e-08, 3.37e-08, cmap="Blues")

    nn = 2000 * ind
    if ind % 1 == 0:
        pg.take_screenshot("%s/D_%d.png" % (path, nn))
fp.close()
